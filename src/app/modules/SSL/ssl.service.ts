...
const initPayment = async (paymentData: any) => {
    try {
        const data = {
            ...
            success_url: config.ssl.sucess_url,
        ...
        throw new ApiError(httpStatus.BAD_GATEWAY, "Payment error occured!")
    }
}
...
const validatePayment = async (payload: any) => {
    try {
        const response = await axios({
            method: "GET",
            url: `${config.ssl.ssl_validation_api}?val_id=${payload.val_id}&store_id${config.ssl.storeId}&store_paswd=${config.ssl.store_pass}&format=json`
        })
        return response.data
    } catch (err) {
